...
export default function SellerOrdersPage() {
  ...
  const fetchOrders = useCallback(async (pageNum = 0) => {
    setLoading(true);
    try {
      const response = await orderService.getSellerOrders({ page: pageNum, size: PAGE_SIZE });
      const data = response;
  ...
  const filteredOrders = useMemo(() => {
    ...
    if (activeTab === 'problem') return orders.filter(o => o.status === ORDER_STATUS.CANCELLED || o.status === ORDER_STATUS.REFUNDED);
    ...
    <div className="min-h-screen flex flex-col bg-[#f9f4ee] font-body text-[#3f3d2e]">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 py-8 md:py-12">
        <div className="mb-6 flex justify-between items-end">
          <div>
            <h1 className="text-3xl font-bold text-[#ac4218] mb-2 tracking-tight">Quản lý Đơn hàng</h1>
            <p className="text-sm text-gray-500 font-medium">Kênh người bán - Theo dõi và xử lý đơn hàng của cửa hàng bạn.</p>
          </div>
        </div>
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-[#fcfaf8] text-[#646652] border-b border-[#ede5db]">
                <tr>
                  ...
                  <th className="px-6 py-4 font-bold tracking-wider">Ngày đặt</th>
                  <th className="px-6 py-4 font-bold tracking-wider">Người mua</th>
                  <th className="px-6 py-4 font-bold tracking-wider">Tổng tiền</th>
              ...
              <tbody className="divide-y divide-[#ede5db]">
                {loading ? (
                  <tr>
                    <td colSpan="6" className="px-6 py-16 text-center text-gray-500 font-medium">Đang tải dữ liệu đơn hàng...</td>
                  </tr>
                ) : filteredOrders.length === 0 ? (
                  <tr>
                    <td colSpan="6" className="px-6 py-16 text-center">
                      <div className="text-5xl opacity-30 mb-4">📦</div>
                      <p className="text-gray-500 font-medium text-base">Không có đơn hàng nào.</p>
                    </td>
                  </tr>
                ) : (
                  filteredOrders.map(order => (
                    <tr key={order.id} className="hover:bg-[#fafafa] transition-colors">
                      <td className="px-6 py-5 font-mono font-bold text-gray-800 uppercase">{order.id}</td>
                      <td className="px-6 py-5 text-gray-600 font-medium">{formatOrderDate(order.createdAt)}</td>
                      <td className="px-6 py-5 text-[#3f3d2e] font-bold">{order.buyerName}</td>
                      <td className="px-6 py-5 font-bold text-brand-600 text-base">{formatPrice(order.totalAmount)}</td>
