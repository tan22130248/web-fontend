...
export default function SellerOrderDetailPage() {
  ...
  if (loading) return <div className="min-h-screen bg-[#f9f4ee] flex items-center justify-center font-body text-gray-500">Đang tải thông tin...</div>;
  if (!order) return <div className="min-h-screen bg-[#f9f4ee] flex items-center justify-center font-body text-gray-800">Không tìm thấy đơn hàng</div>;
    ...
    <div className="min-h-screen flex flex-col bg-[#f9f4ee] font-body text-[#3f3d2e]">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 py-8 md:py-12">
        <div className="flex flex-wrap items-center gap-4 mb-8">
          <button onClick={() => navigate('/seller/orders')} className="text-gray-500 hover:text-gray-900 bg-white border border-gray-200 px-4 py-2 rounded-xl shadow-sm text-sm font-medium transition-colors">
            ← Quay lại
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#ede5db] shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h2 className="text-lg font-bold text-gray-900 mb-1">Xử lý đơn hàng</h2>
                <p className="text-sm text-gray-500">Cập nhật trạng thái để thông báo cho người mua.</p>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                {order.status === ORDER_STATUS.PENDING && (
                  <>
                    ...
                    <button 
                      onClick={() => handleAction(orderService.confirmOrder, 'Đã xác nhận đơn', ORDER_STATUS.CONFIRMED)}
                      disabled={processing}
                ...
                {order.status === ORDER_STATUS.CONFIRMED && (
                  <button 
                    onClick={() => handleAction(orderService.shipOrder, 'Đã giao cho ĐVVC', ORDER_STATUS.SHIPPING)}
                    disabled={processing}
                ...
                {order.status === ORDER_STATUS.SHIPPING && (
                  <button 
                    onClick={() => handleAction(orderService.deliverOrder, 'Giao hàng thành công', ORDER_STATUS.DELIVERED)}
                    disabled={processing}
          ...
          <div className="space-y-6 lg:space-y-8">
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#ede5db] shadow-sm sticky top-6">
              ...
              <div className="text-sm space-y-3 bg-[#fcfaf8] p-4 rounded-xl border border-gray-100">
                <p className="font-bold text-gray-800 text-base">{order.buyerName}</p>
                {order.buyerEmail && (
              ...
              <div className="mt-8">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center pt-4 border-t border-gray-100 text-base">
                    <span className="font-bold text-gray-700">Khách phải trả</span>
      ...
      <CancelOrderDialog 
        ...
        reasons={['Hết hàng', 'Không thể giao đến khu vực này', 'Sai thông tin giá', 'Lý do khác']}
      />
    </div>
  );
}
