...
export default function SellerAnalytics() {
  ...
  const getRankInfo = (points) => {
    if (points >= 2000) return { name: 'Hạng Vàng', color: 'bg-yellow-100 text-yellow-700', nextLevel: 5000 };
    if (points >= 1000) return { name: 'Hạng Bạc', color: 'bg-gray-200 text-gray-700', nextLevel: 2000 };
    return { name: 'Hạng Đồng', color: 'bg-[#EDE9DA] text-gray-600', nextLevel: 1000 };
  };
...
<<<<<<< HEAD
      ...
      <div className="flex flex-wrap gap-4">
        <div className="bg-[#EBF5E6] text-[#2E4213] p-4 rounded-xl flex-1 min-w-[200px] border border-[#D5E8CD]">
           <div className="text-[10px] font-bold uppercase tracking-wider mb-1">Đơn hàng chờ xử lý</div>
        ...
        <div className="bg-[#F0F4F8] text-[#2C3E50] p-4 rounded-xl flex-1 min-w-[200px] border border-[#DCE4EC]">
           <div className="text-[10px] font-bold uppercase tracking-wider mb-1">Sản phẩm đang bán</div>
           <div className="text-2xl font-black">{dashboardData.productsOnline}</div>
        </div>
      </div>
          ...
          <div key={idx} className="bg-white p-4 rounded-2xl border border-[#F0ECE0] shadow-sm relative overflow-hidden flex flex-col justify-between min-h-[105px]">
            <div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-gray-400 tracking-wide">{item.label}</span>
                {item.change && (
...
>>>>>>> main
      <div className="bg-white rounded-2xl border border-[#F0ECE0] shadow-sm overflow-hidden">
        <div className="p-5 border-b border-[#FAF8F0]">
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">Ghi nhận các hoạt động ảnh hưởng đến điểm số của cửa hàng.</p>
        </div>
