...
import { ORDER_STATUS, formatPrice, formatOrderDate } from '../../utils/orderUtils';
...
function buildTimelineFromStatus(status) {
  const steps = [
    { label: 'Đơn hàng đã đặt', note: 'Chờ người bán xác nhận', done: true, active: false },
    { label: 'Đã xác nhận', note: 'Người bán đang chuẩn bị hàng', done: false, active: false },
    { label: 'Đang giao', note: 'Đơn hàng đã được giao cho đơn vị vận chuyển', done: false, active: false },
    { label: 'Đã giao', note: 'Giao hàng thành công', done: false, active: false }
  ];

  if (status === ORDER_STATUS.CANCELLED) {
    return [
      { label: 'Đơn hàng đã đặt', done: true },
      { label: 'Đã huỷ', note: 'Đơn hàng đã bị huỷ', active: true, done: false }
    ];
  }
  ...
  if (status === ORDER_STATUS.REFUNDED) {
    return [
      ...
      { label: 'Hoàn tiền', note: 'Đã xử lý hoàn tiền', active: true, done: false }
    ];
  }

  const statusOrder = [ORDER_STATUS.PENDING, ORDER_STATUS.CONFIRMED, ORDER_STATUS.SHIPPING, ORDER_STATUS.DELIVERED];
...
export default function OrderDetailPage() {
  ...
  if (loading) return <div className="min-h-screen bg-[#f9f4ee] flex items-center justify-center text-gray-500 font-body">Đang tải thông tin...</div>;
  if (!order) return <div className="min-h-screen bg-[#f9f4ee] flex items-center justify-center font-body">Không tìm thấy đơn hàng</div>;

  // Build timeline — prefer API history[], fallback to status-based
  const timelineSteps = (order.history && order.history.length > 0)
  ...
  // Payment display
  const paymentLabel = order.type === 'cod' ? 'Thanh toán khi nhận hàng' : (order.type || 'COD');
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        <div className="flex flex-wrap items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-3 ml-auto">
            <span className="text-sm font-medium text-gray-500">Mã đơn: <span className="text-gray-800 font-bold uppercase">{order.id}</span></span>
          ...
          <div className="lg:col-span-2 space-y-6 lg:space-y-8">
            ...
            {order.shopName && (
              <div className="bg-white rounded-3xl px-6 py-4 border border-[#ede5db] shadow-sm flex items-center gap-3">
                {order.shopAvatarUrl && (
                  <img src={order.shopAvatarUrl} alt={order.shopName} className="w-8 h-8 rounded-full object-cover" />
                )}
                <span className="font-bold text-[#3f3d2e]">{order.shopName}</span>
              </div>
            )}
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#ede5db] shadow-sm">
              ...
              <div className="space-y-4 text-sm max-w-md mx-auto sm:mx-0 sm:ml-auto">
                <div className="flex justify-between items-center text-gray-600">
                  <span>Tổng tiền hàng</span>
                  <span className="font-medium text-gray-900">{formatPrice(order.totalAmount - (order.shippingFee || 0))}</span>
                </div>
                <div className="flex justify-between items-center text-gray-600">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-gray-100 text-base">
                  ...
                  <span className="font-bold text-[#ac4218] text-xl">{formatPrice(order.totalAmount)}</span>
                </div>
          ...
          <div className="space-y-6 lg:space-y-8">
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-[#ede5db] shadow-sm sticky top-6">
              ...
              <div className="pt-6 border-t border-gray-100">
                <h3 className="text-sm font-bold text-gray-700 mb-4 uppercase tracking-wider">Giao hàng đến</h3>
                <div className="bg-[#f9f4ee] p-4 rounded-xl border border-[#ede5db]">
                  <p className="font-bold text-gray-800 text-sm">{order.buyerName}</p>
                  {order.buyerEmail && <p className="text-gray-600 text-sm mt-1">{order.buyerEmail}</p>}
                  <p className="text-gray-500 text-sm mt-2 leading-relaxed">{order.shippingAddress}</p>
                </div>
              </div>
