...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    if (filters.keyword && filters.keyword.trim() !== "") {
      ...
      productService
        ...
        .catch((error) =>
          console.error("Lỗi khi lọc danh sách sản phẩm:", error),
        );
    }
  }, [filters]);
  ...
  const handleConditionChange = (status) => {
    setFilters((prev) => ({
      ...prev,
      conditionStatus: prev.conditionStatus === status ? "" : status,
      page: 0,
    }));
  };
          ...
          <div className="mb-6">
            ...
            <div className="space-y-2 text-sm text-gray-600">
              {conditions.map((status) => (
                <label
                  key={status}
                  className="flex items-center space-x-2 cursor-pointer uppercase"
                >
                  <input
                    type="checkbox"
                  ...
                  <span
                    className={
                      filters.conditionStatus === status
                        ? "font-medium text-[#4A3B32]"
                        : ""
                    }
                  >
            ...
            {products.map((product) => (
              ...
              >
                ...
                <div className="p-4 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="font-semibold text-sm text-[#4A3B32] line-clamp-1 mb-1">
                      {product.name}
                    </h3>
                    <p className="font-bold text-base text-[#A14A24]">
                  ...
                  <div className="mt-4 pt-3 border-t border-[#F7F5EE] flex items-center justify-between text-[11px] text-gray-500">
                    <div className="flex items-center space-x-2">
                      <div className="w-6 h-6 rounded-full bg-orange-300 text-white flex items-center justify-center font-bold text-[9px]">
                        {product.shopName
                          ? product.shopName.substring(0, 2).toUpperCase()
                          : "TC"}
                      </div>
                      <p className="font-medium text-[#4A3B32]">
                        {product.shopName || "Tiệm đồ cũ"}
                      </p>
                    </div>
      ...
      <footer className="bg-[#F5F2E6] border-t border-[#EBE7D9] mt-20 py-10 text-center">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <p className="text-[11px] text-gray-400">
            © 2026 Tiệm Cũ - Chạm vào ký ức, sẻ chia phong cách.
          </p>
        </div>
      </footer>
    </div>
